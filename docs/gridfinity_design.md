...
* **CI pipeline** (GitHub Actions) that converts every `*.scad` into a binary STL artifact on each push and publishes them as build outputs.
...
| artifacts | Upload each rendered STL; optionally commit to stl/YYYY/ on main via ad-m/github-push-action@v0.8.0. |
