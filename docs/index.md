...
- [Product and engineering design](product-design.md) – browser MVP, physical-validation status, scene and future hosted contracts.
- [Roadmap](roadmap.md) – measurable phases from this browser release through physical calibration and hosting.
- [Release guide](releasing.md) – immutable image/chart publication, staging expectations, and rollback.

## Browser MVP

The maintained viewer has moved from the CDN-based `viewer.html` prototype to the
Vite application under `web/`; the old page is a compatibility/migration notice.
Run `cd web && npm ci && npm run dev`, optionally prepare exact canonical assets
with `npm run prepare:models`, and use local file pickers for metadata and STL
bundles. Nothing is uploaded. Proxy geometry is labeled **Design preview** and is
never offered as printable STL; exact loaded/generated bytes are labeled **Exact
STL geometry** and can be downloaded unchanged along with the active print
manifest. The text mode preserves every month, contribution count, cube height,
quantity, position, file classification, and download state without WebGL.

The deployable service exposes `/`, `/healthz`, and `/livez`. Live GitHub retrieval,
authentication, runtime generation, persistence, and production routing are absent
from this opening MVP. `staging.gitshelves.com` is planned; Sugarkube overlays and
Cloudflare DNS/Tunnel configuration remain external.
