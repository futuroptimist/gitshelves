...
def _iter_monthly_block_positions(
    ...
    """Yield ``(x, y, z, year, month, level)`` for monthly contributions."""
    if months_per_row <= 0:
...
def generate_scad_monthly(
    ...
    """
    scad_lines = [HEADER]
    for x, y, z, year, month, _ in _iter_monthly_block_positions(
        contributions, months_per_row
    ):
        scad_lines.append(
...
def generate_scad_monthly_levels(
    ...
    for x, y, z, year, month, level in _iter_monthly_block_positions(
        contributions, months_per_row
    ):
