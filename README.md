...
`--calendar-days-per-row`, the CLI mirrors the monthly grid width up to the
twelve-month span (`monthly-12x6` with the default layout). Pass `--calendar-days-per-row`
...
calendar mirrors the monthly grid width when `--calendar-days-per-row` is omitted, producing a
default twelve-column layout that still fits within a 256 mm square build area (`monthly-12x6`). Use
