...
cube counts plus a fresh copy of the bundled `baseplate_2x6.scad`. The CLI always renders
`baseplate_2x6.stl` alongside the summary so the standard Gridfinity plate is ready to print without
extra commands. Install OpenSCAD before running the CLI—the automatic baseplate render depends on
it. The CLI writes these summaries and baseplates for every year in the requested range, even when a
year has no contributions, so your shelf layout stays predictable.
