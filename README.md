...
The deployable browser MVP opens with a clearly synthetic twelve-month boundary sample on the existing 2×6 monthly base. Orbit, pan, zoom, fit/reset, assembled/exploded modes, color visibility, and a complete text print plan work without a GitHub login. Import existing GitShelves metadata/run-summary JSON and associated local STL files; nothing is uploaded and this PR performs **no live GitHub retrieval**. Imported STL bytes are preserved for unchanged download, while the manifest reports month placements, heights, totals, colors, file references, and cautious assembly guidance.

The status distinguishes metadata-driven **Design preview** proxy boxes (never printable downloads) from **Exact STL geometry** generated by canonical Python/OpenSCAD or loaded locally. Production builds contain exact canonical base and reusable stackable module assets. An unprepared dev session remains useful but disables those downloads and explains preparation.
...
cd ..
scripts/fetch_gridfinity.sh  # pinned library revision
python scripts/prepare_web_models.py
cd web && npm run build
```

The legacy [`docs/viewer.html`](docs/viewer.html) remains available for existing standalone local-file workflows; the Vite app is its compatibility successor and preserves baseplate, `_colorN`, legacy `levelN`, STLLoader, OrbitControls, and color-visibility behavior.

The production container serves `/`, `/healthz`, and `/livez` on port 8080. See the [product design](docs/product-design.md), [roadmap](docs/roadmap.md), and [release guide](docs/releasing.md). `staging.gitshelves.com` is planned only: Sugarkube deployment overlays and Cloudflare DNS/Tunnel configuration are external and are not changed here. The monthly 2×6 product is retained because it is the smallest layout already aligned across metadata, SCAD, STL, and assembly contracts; daily 53×7 carriers are future design work.
