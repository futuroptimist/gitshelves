...
def test_cli_multiple_colors_without_stl(tmp_path, monkeypatch, capsys):
    ...
    def record_stl(src, dest):
        src_path = Path(src)
        dest_path = Path(dest)
        called.append((src_path.name, dest_path.name))
        dest_path.write_text("// stl")

    monkeypatch.setattr(cli, "scad_to_stl", record_stl)
    monkeypatch.setattr(
    ...
    assert baseplate_scad.read_text() == "// Baseplate"
    assert called == [("baseplate_2x6.scad", "baseplate_2x6.stl")]
...
def test_cli_writes_yearly_baseplate_scad(tmp_path, monkeypatch):
    ...
    stl_path = baseplate.with_suffix(".stl")
    names = [(src.name, dest.name) for src, dest in calls]
    assert ("baseplate_2x6.scad", "baseplate_2x6.stl") in names
...
def test_cli_generates_gridfinity_cubes(tmp_path, monkeypatch, gridfinity_library):
    ...
    def record_stl(src, dest):
        src_path = Path(src)
        dest_path = Path(dest)
        calls.append((src_path.name, dest_path.name))
        dest_path.write_text("// stl")
    ...
    baseplate_stl = year_dir / "baseplate_2x6.stl"
    assert ("baseplate_2x6.scad", "baseplate_2x6.stl") in calls
    assert baseplate_stl.exists()
    assert len(calls) == 1
...
def test_cli_readme_notes_empty_gridfinity_cubes(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        cli,
        "scad_to_stl",
        lambda src, dest: Path(dest).write_text("// stl"),
    )
