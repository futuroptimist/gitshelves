...
def test_cli_multicolor_removes_single_color_outputs(tmp_path, monkeypatch, capsys):
    """Multi-color runs should delete stale single-color exports as documented."""

    single_scad = tmp_path / "chart.scad"
    single_scad.write_text("// previous single-color run", encoding="utf-8")
    single_stl = tmp_path / "chart.stl"
    single_stl.write_text("stl", encoding="utf-8")
    single_scad.with_suffix(".json").write_text(
        json.dumps({"stl": str(single_stl), "stl_generated": True}),
        encoding="utf-8",
    )
    ...
    def fake_levels(counts, months_per_row=12):
        assert months_per_row == 12
        return {1: "// Generated by g\nL1"}

    monkeypatch.setattr(cli, "generate_scad_monthly_levels", fake_levels)
    monkeypatch.setattr(cli, "generate_zero_month_annotations", lambda *a, **k: [])
    monkeypatch.setattr(
        cli, "load_baseplate_scad", lambda name="baseplate_2x6.scad": "// baseplate"
    )
    ...
    def fake_stl(src, dest):
        rendered.append((Path(src), Path(dest)))
    ...
    captured = capsys.readouterr()
    assert f"Wrote {tmp_path / 'chart_baseplate.scad'}" in captured.out

    assert not single_scad.exists(), "Combined single-color SCAD should be removed"
    assert not single_stl.exists(), "Combined single-color STL should be removed"
    assert not single_scad.with_suffix(".json").exists(), "Metadata should be cleared"

    color_outputs = sorted(tmp_path.glob("chart_color*.scad"))
    assert color_outputs, "Expected multi-color SCAD outputs"

    rendered_names = {dest.name for _, dest in rendered}
    assert "chart_color1.stl" in rendered_names
    assert "chart.stl" not in rendered_names, "Combined STL should not be regenerated"
...
def test_cli_multicolor_removes_single_color_export(tmp_path, monkeypatch):
    """Multi-color runs should drop the unified SCAD/metadata from previous runs."""

    combined = tmp_path / "combo.scad"
    combined.write_text("// stale")
    metadata = combined.with_suffix(".json")
    old_stl = combined.with_suffix(".stl")
    metadata.write_text(json.dumps({"stl": str(old_stl)}))
    old_stl.write_text("stale stl", encoding="utf-8")
    ...
    monkeypatch.setattr(
        ...
        lambda levels, groups: (levels, {idx: [idx] for idx in levels}),
    )
    monkeypatch.setattr(
        cli,
        "generate_zero_month_annotations",
    ...
    monkeypatch.setattr(cli, "load_baseplate_scad", lambda *_: "// baseplate")
    monkeypatch.setattr(
        cli,
        "generate_monthly_calendar_scads",
        lambda *_args, **_kwargs: {month: "//" for month in range(1, 13)},
    )
    ...
    def fake_write_year_readme(
        ...
        *,
        ...
        year_dir = tmp_path / "stl" / str(year)
        year_dir.mkdir(parents=True, exist_ok=True)
        path = year_dir / "README.md"
    ...
    assert not combined.exists(), "Unified SCAD should be removed on multi-color runs"
    assert not metadata.exists(), "Single-color metadata should be deleted"
    assert not old_stl.exists(), "Previous single-color STL should be removed"
    color_outputs = sorted(tmp_path.glob("combo_color*.scad"))
    assert color_outputs, "Color-specific SCAD outputs should be generated"
...
def test_cli_multi_color_removes_orphan_color_metadata(tmp_path, monkeypatch):
    """Palette shrinks should drop leftover `_colorN.json` metadata files."""
    ...
    stale_metadata = tmp_path / "palette_color3.json"
    stale_metadata.write_text(
        json.dumps(
            {
                "scad": str(tmp_path / "palette_color3.scad"),
                "stl": str(tmp_path / "palette_color3.stl"),
                "stl_generated": True,
            }
        )
    )
    stale_stl = tmp_path / "palette_color3.stl"
    stale_stl.write_text("old", encoding="utf-8")
    ...
    assert not stale_metadata.exists(), "Stale color metadata should be removed"
    assert not stale_stl.exists(), "Associated stale STL should also be purged"
