...
def test_generate_scad_monthly_marks_zero_contribution_months():
    counts = {(2024, 1): 0, (2024, 2): 15}
    scad = generate_scad_monthly(counts)
...
def test_generate_month_calendar_scad_marks_zero_contribution_days():
    scad = generate_month_calendar_scad({}, 2024, 2)
    assert "// 2024-02-01 (0 contributions) reserved at [0, 0]" in scad
    assert "// 2024-02-05 (0 contributions) reserved at [48, 0]" in scad
...
def test_generate_monthly_calendar_scads_returns_all_months():
    ...
    assert "2023-01-01 (0 contributions)" in calendars[1]
...
def test_group_scad_levels_with_mapping_tracks_levels():
    levels = {
        1: "// Generated by g\nL1",
        2: "// Generated by g\nL2",
        3: "// Generated by g\nL3",
    ...
    grouped, mapping = group_scad_levels_with_mapping(levels, 3)

    assert mapping == {1: [1, 2], 2: [3], 3: [4]}
    assert grouped[1].startswith("// Generated by gitshelves")


def test_group_scad_levels_with_mapping_accent_group_collects_remaining():
    levels = {idx: f"// Generated by g\nL{idx}" for idx in range(1, 6)}

    grouped, mapping = group_scad_levels_with_mapping(levels, 4)

    assert mapping[4] == [4, 5]
    assert len(mapping[1]) <= 1


def test_group_scad_levels_balances_three_groups():
    """Levels should be balanced across groups when fewer than four colors."""
    ...
    grouped = group_scad_levels(levels, 3)
...
def test_group_scad_levels_balances_two_groups():
    levels = {
        1: "// Generated by g\nL1",
        2: "// Generated by g\nL2",
        3: "// Generated by g\nL3",
...
def test_group_scad_levels_fourth_group_collects_remaining_levels():
    """README promises fourth color collects the highest magnitudes."""
    ...
    assert grouped[1].splitlines()[1:] == ["L1"]
    assert grouped[2].splitlines()[1:] == ["L2"]
    assert grouped[3].splitlines()[1:] == ["L3"]
    assert grouped[4].splitlines()[1:] == ["L4", "L5", "L6"]
...
def test_group_scad_levels_rolls_extra_levels_into_last_group():
    """Higher orders of magnitude should reuse the accent color group."""
...
def test_group_scad_levels_more_than_four_groups_preserve_accent():
    """Levels beyond the first three roll into the last accent group."""
    ...
    assert grouped[2].splitlines()[1:] == ["L2"]
    assert grouped[3].splitlines()[1:] == ["L3"]
    assert grouped[4].splitlines()[1:] == ["L4"]
    assert grouped[5].splitlines()[1:] == ["L5", "L6", "L7"]


def test_group_scad_levels_sparse_levels_still_use_accent_color():
    """Accent color should keep the highest magnitudes even when gaps exist."""
    ...
    assert grouped[1].splitlines()[1:] == ["L1"]
    assert grouped[2].splitlines()[1:] == []
    assert grouped[3].splitlines()[1:] == []
    assert grouped[4].splitlines()[1:] == ["L4", "L6"]
